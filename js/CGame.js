function CGame(oData){
    ...
    this._checkWin = function(iMaxWinIndex){
        ...
        console.log('==================', _aListSelected, WIN_OCCURRENCE, _aNumSelected);
    ...
    this._animExtraction = function(){        
        ...
        console.log((_aCell[ (_aCombination[0]) ].getGlobalPosition()));
        for(var i=0; i<20; i++){
    ...
    this._checkContinueGame = function(){        
        ...
        for(var i=0; i<PAYOUTS[_iTotalNum-1].hits.length; i++){
            if(PAYOUTS[_iTotalNum-1].hits[i] === _iHitsNumber){
                iTotalWin = (_iCurBet*PAYOUTS[_iTotalNum-1].pays[i]);
        ...
        $(s_oMain).trigger("save_score", [_iCurPlayerMoney, iTotalWin]);
        if(_iCurBet>_iCurPlayerMoney){
